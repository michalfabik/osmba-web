...
class File
{
  ...
  public function previewLink($zoom = null)
  {
    if (!$this->dimensions) {
      return Environment::getHttpRequest()
        ->getUrl()
        ->getBasePath() . "static/icons/icons48.php?file=$this->suffix";
    }
  ...
  public function getPreviewHttpResponse($o = null)
  {
    ...
    if (!isset($opts["nocache"])) {
      $image->save($cachePath, Image::PNG);
    }
  ...
  {
    if ($opts) {
      return Environment::getVariable("dataDir") .
        "/thumbs/$this->id.$opts.png";
    }
  ...
  public function downloadLink($absolute = false)
  {
    ...
    return Environment::getApplication()
      ->getPresenter()
      ->link($absolute . ':Front:Files:default', array(
        $this->id,
        'lang' => null
      ));
  }
  ...
  public function getDownloadPath()
  {
    if ($this->origpath) {
      ...
      return Environment::getVariable("dataDir") .
        "/files/$this->id.orig.$this->suffix";
    }
  }
  ...
  {
    //inject $httprequest, $router, $params
    if (!$data) {
  ...
  public function save($newdata = false)
  {
    ...
    if ($newdata) {
      foreach (
        $newdata
        as $k =>
          $v //merge changes to orig data array
      ) {
        $this->data[$k] = $v;
      }
    }
    ...
    if (isset($opts["fadeframe"])) {
      ...
      for ($i = 0; $i < $size; $i++) {
        $image->line(
          ...
          $w,
          $i,
          Image::rgb(255, 255, 255, round((127 / $size) * $i))
        );
        $image->line(
          0,
          $h - $i,
...
class ImageFile extends File
{
   ...
   */
  static function parseIPTC($info)
  {
    //TODO rozpoznávátko na codepage :-/
...
class MapsFile extends File
{
  ...
  public static function saveGoogleMapsPreview($f)
  {
    //TODO show also markers
    ...
    if ($f->suffix == 'gpx') {
      ...
      foreach ($res as $r) {
        $points[] = array(
          floatval((string) $r['lat']),
          floatval((string) $r['lon'])
        );
      }
    } elseif ($f->suffix == 'kml') {
    ...
    for ($x = 0.00001; strlen($enc[2]) > 1500; $x *= 2) {
      //url limit is 2000
      $gp->verySmall = $x;
      $enc = $gp->dpEncode($points);
    }
    ...
    $url =
      "http://maps.googleapis.com/maps/api/staticmap?size=120x120&path=weight:3%7Ccolor:blue%7Cenc:" .
      $enc[2] .
      "&sensor=false";
...
class DocumentFile extends File
{
  ...
  {
    $url =
      Environment::getHttpRequest()
        ->getUrl()
        ->getHostUrl() . $this->downloadLink(); //absolute URL
  ...
  public static function getGoogleViewerPreview($url)
  {
    ...
    $image = file_get_contents(
      "http://docs.google.com/viewer$biUrl[1]&pagenumber=1&w=600"
    );
...
class SoundFile extends File
{
  ...
  public function getControlHtml($opts = null)
  {
    ...
    $link =
      Environment::getHttpRequest()
        ->getUrl()
        ->getBasePath() . "data/files/$this->id.orig.$this->suffix";
...
class VideoFile extends File
{
  ...
  public function getControlHtml($opts = null)
  {
    ...
    // custom templating
    $template =
      Environment::getContext()->params["themeDir"] . '/control-video.latte';
    if (file_exists($template)) {
      ...
      $tpl->preview = $preview;
      $tpl->link = $link;
      $tpl->basePath = Environment::getHttpRequest()
        ->getUrl()
        ->getBasePath();
      return (string) $tpl;
    }
...
class MissingFile extends File
{
  ...
  public function previewLink($zoom = null)
  {
    return Environment::getHttpRequest()
      ->getUrl()
      ->getBasePath() . "static/images/missing-image.png";
  }
