...
class Front_TalkbaPresenter extends Front_BasePresenter
{
  public function startup()
  {
    ...
    if ($this->isAjax()) {
      ...
      $this->payload->uri = $this->context->httpRequest
        ->getUrl()
        ->getRelativeUrl();
    }
  }
  ...
  public function actionDefault($month = null)
  {
    if (!$month or !preg_match("~^(\d{4})(\d{2})$~", $month, $matches)) {
      $defaultMonth = dibi::fetchSingle(
        "SELECT DATE_FORMAT(max(`date`), '%Y%m') FROM mailarchive"
      );
  ...
  public function actionConversation($id)
  {
    ...
    $this->template->minYear =
      $min->format('Y') == $max->format('Y') ? "" : $min->format('Y');
  ...
  public function actionAuthor($stub)
  {
    ...
    $mailList = dibi::query(
      "SELECT * FROM mailarchive WHERE `from` = %s",
      $author->from,
      " ORDER BY `date` DESC"
    );
  ...
  public function actionSearch($query)
  {
    $result = dibi::query(
      ...
      $query,
      " IN BOOLEAN MODE)
		    ORDER BY date DESC, MATCH(text) AGAINST (%s",
      $query,
      ") DESC
		    LIMIT 501
	    "
