...
class Helpers
{
  ...
  public static function talkMailBody($s)
  {
    ...
    if ($opened) {
      $out[$opened] =
        "<div style='border-left:1px silver solid;padding-left:5px'>" .
        $out[$opened];
      $out[count($out) - 1] .= "</div>";
    }
    ...
    $s = preg_replace(
      '~(https?://)([^ \n\r\t()[\]]+)~is',
      '<a href="\\1\\2" target="_blank" rel="nofollow noopener">\\1\\2</a>',
      $s
    );
    $s = str_replace(")&gt;", "&gt;", $s);
    ...
    if ($delta < 0) {
      ...
      if ($delta < 45) {
        return 'za ' .
          $delta .
          ' ' .
          self::plural($delta, 'minuta', 'minuty', 'minut');
      }
      if ($delta < 90) {
        return 'za hodinu';
      }
      ...
      return 'za ' .
        round($delta / 525960) .
        ' ' .
        self::plural(round($delta / 525960), 'rok', 'roky', 'let');
    }
